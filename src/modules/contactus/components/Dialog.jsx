...
import { useDeleteContactus } from "hooks/contactus/useDeleteContactus";
...
const DeleteDialog = ({ id, page, count }) => {
  ...
  const deletecontactus = useDeleteContactus({ page, count });
  const handleClickOpen = (e) => setOpen(true)
  const handleClose = () => setOpen(false)
  const { refetch } = useContactus();
  const DeleteContactus = () => {
    setLoading(true);
    deletecontactus.mutate(id, {
      onSuccess: () => {
        setOpen(false);
        refetch()
      },
    ...
    <React.Fragment>
      ...
      >
        ...
        <DialogActions>
          ...
          {loading && <Loader />}
          <Button autoFocus sx={{}} variant="contained" onClick={DeleteContactus}>
            {t("Agree")}
          </Button>
        </DialogActions>
      </Dialog>
