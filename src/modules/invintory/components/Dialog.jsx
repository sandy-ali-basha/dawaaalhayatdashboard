...
import { useDeleteInvintory } from "hooks/invintory/useDeleteInvintory";
...
const DeleteDialog = ({ id, page, count }) => {
  ...
  const deleteinvintory = useDeleteInvintory({ page, count });
  const handleClickOpen = (e) => setOpen(true)
  const handleClose = () => setOpen(false)
  const { refetch } = useInvintory();
  const DeleteInvintory = () => {
    setLoading(true);
    deleteinvintory.mutate(id, {
      onSuccess: () => {
        setOpen(false);
        refetch()
      },
    ...
    <React.Fragment>
      ...
      >
        ...
        <DialogActions>
          ...
          {loading && <Loader />}
          <Button autoFocus sx={{}} variant="contained" onClick={DeleteInvintory}>
            {t("Agree")}
          </Button>
        </DialogActions>
      </Dialog>
