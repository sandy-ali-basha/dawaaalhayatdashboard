...
import { useDeleteAboutus } from "hooks/aboutus/useDeleteAboutus";
...
const DeleteDialog = ({ id, page, count }) => {
  ...
  const deleteaboutus = useDeleteAboutus({ page, count });
  const handleClickOpen = (e) => setOpen(true)
  const handleClose = () => setOpen(false)
  const { refetch } = useAboutus();
  const DeleteAboutus = () => {
    setLoading(true);
    ...
    <React.Fragment>
      ...
      >
        ...
        <DialogActions>
          ...
          {loading && <Loader />}
          <Button autoFocus sx={{}} variant="contained" onClick={DeleteAboutus}>
            {t("Agree")}
          </Button>
        </DialogActions>
      </Dialog>
